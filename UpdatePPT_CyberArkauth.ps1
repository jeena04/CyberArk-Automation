...
#User who execute this should use Cyberark authentication method.
...
try
{
    ...
    # Entering credentials to login using Cyberark Authentication 
...
$token = Invoke-RestMethod 'baseurl/API/auth/Cyberark/Logon' -Method 'POST' -Headers $headers -Body $body
    $token | ConvertTo-Json
    Add-Content -Path $logFilePathName -Value "AccessToken created successfully"
 
    # User list
    $userIdsFilePath = "input file"
